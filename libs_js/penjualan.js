...
function editData(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				url = 'update_item.php?id_barang='+row.id_barang;
				}
		}
		function saveData(){
			$('#fm').form('submit',{
		...
		function removeData(){
			...
			if (row){
				$.messager.confirm('Confirm','Anda yakin akan menghapus data ini?',function(r){
					if (r){
						$.post('remove_item.php',{id_barang:row.id_barang},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
